...
function ProgressTrack({ className, ...props }: ProgressPrimitive.Track.Props) {
  return (
    <ProgressPrimitive.Track
      className={cn(
        "relative flex h-1.5 w-full items-center overflow-x-hidden rounded-full bg-chart-grid",
        className
      )}
...
function ProgressIndicator({
  ...
  return (
    <ProgressPrimitive.Indicator
      ...
      className={cn("h-full rounded-full bg-chart-2 transition-all", className)}
      {...props}
    />
  )
}
