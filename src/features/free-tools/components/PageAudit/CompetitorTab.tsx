...
function ComparisonBar({ label, you, competitor }: { label: string; you: number; competitor: number }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex flex-1 flex-col gap-1">
        ...
        <div className="h-2 overflow-hidden rounded-full bg-muted">
          <div className="h-full rounded-full bg-red-500" style={{ width: `${competitor}%` }} />
        </div>
      </div>
    </div>
  );
...
export function CompetitorTab({
  ...
  const handleAnalyze = () => {
    ...
    competitorAudit.mutate({ url: yourUrl, competitorUrl: competitorUrl.trim() });
  };
    ...
    <div className="flex flex-col gap-4">
      <SectionCard title="Analyze & Compare a Competitor">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          ...
          <Button onClick={handleAnalyze} disabled={competitorAudit.isPending || !canSubmit} className="h-9">
            {competitorAudit.isPending ? <Loader2 className="size-4 animate-spin" /> : <Search className="size-4" />}
            Analyze
          </Button>
        </div>
        {!yourUrl ? (
          <p className="mt-2 text-[12.5px] text-muted-foreground">Run an audit on your own page first.</p>
        ) : null}
        {competitorAudit.isError || competitorAudit.isSuccess ? (
          !comparison ? (
            <p className="mt-2 text-[12.5px] text-destructive">
              Couldn't read one of the pages — it may be blocking automated requests. Try again.
            </p>
          ) : null
        ) : null}
      </SectionCard>
      ...
      {comparison ? (
          ...
          <SectionCard title="Competitor SEO Score">
            <div className="flex flex-col items-center gap-4">
              <div className="flex items-center gap-6">
                <ScoreRing score={comparison.yourScore} label="You" color="#f97316" />
                <ScoreRing score={comparison.competitorScore} label="Competitor" color="#f97316" />
              </div>
              <div className="flex items-center gap-4 text-[12.5px] text-muted-foreground">
                ...
                <span className="flex items-center gap-1.5">
                  <span className="size-2 rounded-full bg-red-500" /> Competitor
                </span>
              </div>
            ...
            <div className="flex flex-col gap-4">
              <div
                className={`flex items-start gap-2 rounded-lg border p-3 ${
                  comparison.gapPct >= 0 ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50"
                }`}
              >
                {comparison.gapPct >= 0 ? (
                ...
                <div>
                  <p className={`text-[13px] font-medium ${comparison.gapPct >= 0 ? "text-emerald-900" : "text-red-900"}`}>
                    {comparison.gapPct >= 0
                  ...
                  <p className={`text-[12.5px] ${comparison.gapPct >= 0 ? "text-emerald-700" : "text-red-700"}`}>
                    {comparison.gapPct >= 0 ? "+" : ""}
              ...
              <div>
                <p className="mb-2 text-[13px] font-semibold text-foreground">Keyword Usage</p>
                <div className="flex items-center justify-between text-[13px]">
                  <span className="text-foreground">Your occurrences:</span>
                  <span className="font-semibold text-foreground">
                    {comparison.yourOccurrences} ({comparison.yourOccurrencesPct}%)
                  </span>
                </div>
                <div className="mt-1 flex items-center justify-between text-[13px]">
                  <span className="text-foreground">Competitor occurrences:</span>
                  <span className="font-semibold text-foreground">
                    {comparison.competitorOccurrences} ({comparison.competitorOccurrencesPct}%)
                  </span>
                </div>
              </div>
        ...
        <SectionCard title="Competitor">
          <p className="text-[13px] text-muted-foreground">
            Add a competitor URL above and analyze it to see a side-by-side comparison against your page.
          </p>
        </SectionCard>
      )}
    </div>
